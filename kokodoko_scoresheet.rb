#!/usr/bin/ruby
# -*- coding: utf-8 -*-

require 'csv'

o=ARGV[0].to_i

# オプション
# 0 : 通常回のみ
# 1 : 通常回＋特別編
# 2 : 通常回＋特別編＋これはいつでしょう？

a={}

# ここはどこでしょう？
#

file_list = {
  '第1回'  => 'kd_winner_01.txt',
  '第2回'  => 'kd_winner_02.txt',
  '第3回'  => 'kd_winner_03.txt',
  '第5回'  => 'kd_winner_05.txt',
  '第6回'  => 'kd_winner_06.txt',
  '第7回'  => 'kd_winner_07.txt',
  '第8回'  => 'kd_winner_08.txt',
  '第10回' => 'kd_winner_10.txt',
  '第11回' => 'kd_winner_11.txt',
  'いつ第1回' => 'ki_winner_01.txt', # これはいつでしょう？
  'いつ第2回' => 'ki_winner_02.txt', # これはいつでしょう？
  'いつ第3回' => 'ki_winner_03.txt', # これはいつでしょう？
  'KIRIMIちゃん. AB' => 'sp_winner_01.txt',  # ここはどこでしょう？特別編 KIRIMIちゃん.
  'KIRIMIちゃん. CD' => 'sp_winner_02.txt',  # ここはどこでしょう？特別編 KIRIMIちゃん.
  '第13回' => 'kd_winner_13.txt',
  '第14回' => 'kd_winner_14.txt',
  '第15回' => 'kd_winner_15.txt',
  '第16回' => 'kd_winner_16.txt',
  '第18回' => 'kd_winner_18.txt',
  '第19回' => 'kd_winner_19.txt',
  '第20回' => 'kd_winner_20.txt',
  '第21回' => 'kd_winner_21.txt',
  '第23回' => 'kd_winner_23.txt',
  '第24回' => 'kd_winner_24.txt',
  '第25回' => 'kd_winner_25.txt',
  'LIXIL' => 'sp_winner_03.txt',  # LIXIL 特別編
  '第27回' => 'kd_winner_27.txt',
  '第28回' => 'kd_winner_28.txt',
  '第29回' => 'kd_winner_29.txt',
  '第30回' => 'kd_winner_30.txt',
}

file_list.each {|no, fname|
  next if o == 0 and fname =~ /ki|sp/
  next if o == 1 and fname =~ /ki/
  File.open(fname) {|file|
    a[no] = file.read.
            gsub(/（\d{4}\/\d{2}\/\d{2}\s+\d{2}:\d{2}:\d{2}）/, ''). # メール到着時刻を除去
            gsub(/&amp;/, '&'). # &amp; は & に変換
            gsub(/\n/, '') # 改行を除去
  }
}


da=[]
h=Hash.new{|h,k|h[k]={}}
#a.map{|d,s|da<<d;s.split("／").map.with_index(1){|n,i|h[n][d]=i}}
#a.map{|d,s|da<<d;s.split("／").map.with_index(1){|n,i|h[n][d]=i if !h[n].key?(d)}}

#original
#a.map{|d,s|da<<d;s.split("／").map.with_index(1){|n,i|r.map{|x,y|n.gsub!(x,y)};if !h[n].key?(d) then h[n][d]=i else STDERR.puts "dup! #{d}  #{n} : #{i} > #{h[n][d]}" end}}

#read nayose
nr = CSV.read("./nayose.csv")

nrr = {}
nr.map{|t|y=t.shift;t.map{|s|nrr[s]=y}}

#newer sort
a.to_a.reverse.map{|d,s|
  da << d
  s.split("／").map.with_index(1){|n,i|
    nrr.map{|x,y|
      next n.sub!(x,y) if n==x
    }
    # r.map{|x,y|
    #   if n.gsub!(x,y) then STDERR.puts d+" "+x+"→"+y end
    # }
    if !h[n].key?(d) then
      h[n][d]=i
    else
      STDERR.puts "dup! #{d}  #{n} : #{i} > #{h[n][d]}" end
  }
}

#a.to_a.reverse.map{|d,s|da<<d;s.split("／").map.with_index(1){|n,i|r.map{|x,y|if n.gsub!(x,y) then STDERR.puts d+" "+x+"→"+y end};if !h[n].key?(d) then h[n][d]=i else STDERR.puts "dup! #{d}  #{n} : #{i} > #{h[n][d]}" end}}


#a.to_a.reverse.map{|d,s|da<<d;s.split("／").map.with_index(1){|n,i|r.map{|x,y|if(n.gsub!(x,y)) then STDERR.puts x end};if !h[n].key?(d) then h[n][d]=i else STDERR.puts "dup! #{d}  #{n} : #{i} > #{h[n][d]}" end}}

#puts ["名前",da].join(',')
#h.sort_by.with_index{|s,i|[-s[1].size,i]}.map{|n,k|puts [n,k.values_at(*da)].join(',')}

#orignal
#h.sort_by.with_index{|s,i|[-s[1].size,i]}.map{|n,k|s=n.dup;rr.map{|x,y|s.gsub!(x,y)};puts [s,k.values_at(*da)].join(',')}

# 数値だけの場合 ' を先頭に付ける

if ARGV[1] == "sort" then
#name sort
  h.map{|n,k|
    s=n.dup
    # rr.map{|x,y|
    #   s.gsub!(x,y)
    # }
    [s,k.values_at(*da),k.size]
  }.sort_by{|s|
    s
  }.map{|s|
    puts s.join(',')
  }

else
  sep  = ARGV[1] == "table" ? '|' : ','
  ends = ARGV[1] == "table" ? '|' : ''
  quo  = ARGV[1] == "table" ? '' : '"'
  puts sep+''+sep+da.join(sep)+sep if ARGV[1] == "table"
  puts (sep+"---")*(da.size+1)+sep if ARGV[1] == "table"
  #newer sort
  h.sort_by.with_index{|s,i|
    [s[1].values_at,i]
  }.map{|n,k|
    s=n.dup
    # rr.map{|x,y|
    #   s.gsub!(x,y)
    # }
    s = "'"+s if /^[0-9]+$/ =~ s
    puts ends+"#{quo}"+s+"#{quo}"+sep+[k.values_at(*da),k.size].join(sep)+ends
  }

  #h.sort_by.with_index{|s,i|[s[1].values_at,i]}.map{|n,k|s=n.dup;rr.map{|x,y|s.gsub!(x,y)};puts "\""+s+"\","+[k.values_at(*da),k.size].join(',')}

end



#h.sort_by{|k,v|[k.size,k]}.map{|n,k|puts [n,k.values_at(*da)].join(',')}
